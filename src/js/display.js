  ...
  constructor (canvas, bufferWidth, bufferHeight) {
    ...
    this.drawMap = (gameMap, mapColumn) => {
      gameMap.forEach((value, index) => {
        ...
        const sourceX = (value % this.tileSheet.column) * this.tileSheet.tileSize
        const sourceY = Math.floor(value / this.tileSheet.column) * this.tileSheet.tileSize
        // target location to draw tile
        const targetX = (index % mapColumn) * this.tileSheet.tileSize
        const targetY = Math.floor(index / mapColumn) * this.tileSheet.tileSize
        // draw this tile
